...
- 2026-04-19: Renamed visible curriculum sections to lessons, removed zero-padded lesson labels from the UI, added a clickable `Curriculum > Lesson N` breadcrumb on lesson study pages, and introduced curated lesson titles such as `Common verbs with rarely written kanji` for lesson 3.
- 2026-04-19: Tightened lesson review again by blocking `Next verb` and the `Enter` hotkey after a miss until the learner has typed the correct pronunciation, and added a visual-viewport-driven compact mode plus narrower mobile spacing so the revealed lesson layout stays usable on iPhone-sized screens with the keyboard open.
- 2026-04-19: Deferred the lesson-completion redirect until after the final revealed answer card so the last correct submission still shows its full feedback state, and only the next advance returns the learner to the curriculum celebration.
- 2026-04-19: Added an explicit current-lesson state to the curriculum overview by labeling the highlighted lesson with `You are here` and giving it a matching `Start Lesson [enter]` button that follows the learner forward after each completed lesson.
- 2026-04-19: Tightened the lesson header typography so the lesson title sits under `Curriculum > Lesson N:` at the same compact size, taught both the lesson breadcrumb and top-bar brand link to return to a curriculum view centered on the current lesson, and recentered the skipped-status arrow icon inside its circle.
- 2026-04-19: Normalized the revealed lesson action buttons to the same shared control width as the original submit button so `Hear again` and `Next verb` no longer sprawl wider than the prompt controls.
- 2026-04-19: Expanded pronunciation matching to accept IME-converted Japanese verb forms such as `思う` alongside romaji and kana when the typed text resolves to the displayed lesson form, so desktop Japanese keyboards do not incorrectly mark known answers wrong.
- 2026-04-19: Added pair-aware English definition hints so common transitive/intransitive verb families such as `渡す` / `渡る` can render as `... [transitive]` or `... [intransitive]` on study and reference surfaces without cluttering unrelated verbs that merely share a broad gloss.
- 2026-04-20: Collapsed the lesson-study breadcrumb and title into a single inline line so views like `Curriculum > Lesson 7: Daily actions and responsibility` render with one shared visual treatment while keeping `Curriculum` clickable.
- 2026-04-20: Hardened direct lesson deep links by changing the static-host fallback from `sessionStorage`-only route restoration to a URL-carried `?redirect=...` handoff with `sessionStorage` as backup, which avoids the brief lesson-page flash/fallthrough seen in some embedded browsers.
- 2026-04-20: Removed the lesson-input box chrome so the pronunciation field reads like a centered caret-only text line, and shortened the placeholder onboarding cutoff so `Type pronunciation here` disappears after the first 3 completed reviews.
- 2026-04-20: Added learner-oriented speech normalization for kana readings before Web Speech playback, inserting small mora breaks in devoicing-prone sequences such as `すす...` so verbs like `進む` and `進める` are spoken more clearly by native-style system voices.
- 2026-04-20: Replaced the old reveal-side inflection gloss block with compact generated example sentences, removing the redundant `Correct reading` / `Read it as` copy and showing a short Japanese usage line plus an English line instead.
- 2026-04-20: Rewrote `docs/PRD.md` to make the current typed-pronunciation, curriculum-first product the official spec and refreshed `docs/prd-gap-action-plan.md` so it no longer treats the old reveal/self-grade loop as the target behavior.
